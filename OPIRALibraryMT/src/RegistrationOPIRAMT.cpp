...
RegistrationOPIRAMT::~RegistrationOPIRAMT() 
{
	for (unsigned int i=0; i<regOPIRAThreads.size(); i++) {
		regOPIRAThreads.at(i)->stop(); delete regOPIRAThreads.at(i);
	}
	regOPIRAThreads.clear();

	regThread->stop(); delete regThread;
}
...
bool RegistrationOPIRAMT::addMarker(string markerName) {
	...
	if (regThread->addMarker(marker)) {
		...
		RegistrationThreadOPIRA *r = new RegistrationThreadOPIRA(regAlgorithm->clone(), marker.size);
		r->addMarker(marker);
		regOPIRAThreads.push_back(r);
