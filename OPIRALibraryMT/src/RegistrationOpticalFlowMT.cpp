...
RegistrationOpticalFlowMT::RegistrationOpticalFlowMT(RegistrationAlgorithm *registrationAlgorithm):RegistrationOpticalFlow(registrationAlgorithm) 
{
	regThread = new RegistrationThread(registrationAlgorithm->clone());
}

RegistrationOpticalFlowMT::~RegistrationOpticalFlowMT() 
{
	regThread->stop();
	delete regThread;
}
...
bool RegistrationOpticalFlowMT::removeMarker(string markerName) {
...
bool RegistrationOpticalFlowMT::addMarker(string markerName) {
...
bool RegistrationOpticalFlowMT::addResizedMarker(string markerName, int maxLengthSize) {
	//Create a new marker object and set it up
...
bool RegistrationOpticalFlowMT::addScaledMarker(string markerName, int maxLengthScale) {
	//Create a new marker object and set it up
	Marker marker; marker.image = cvLoadImage(markerName.c_str());
	if (marker.image==0) { std::cerr << "Cannot Load marker: " << markerName << endl; return false; }
	marker.name = markerName;
...
bool RegistrationOpticalFlowMT::addResizedScaledMarker(string markerName, int maxLengthSize, int maxLengthScale) {
	//Create a new marker object and set it up
	Marker marker; IplImage *origImage	= cvLoadImage(markerName.c_str());
	if (origImage==0) { std::cerr << "Cannot Load marker: " << markerName << endl; return false; }
...
vector<MarkerTransform> RegistrationOpticalFlowMT::performRegistration(IplImage* frame_input, CvMat* captureParams, CvMat* captureDistortion) 
	{
	...
	for (unsigned int i=0; i<matches.size(); i++) {
		...
		if (bestMatch !=0 && i<previousMatches.size()) {
